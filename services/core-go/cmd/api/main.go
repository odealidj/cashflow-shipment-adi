...
func main() {
	...
	// Initialize Services (Two-Tier Session & Redis Caching)
	cacheService := repository.NewRedisCacheService(redisClient)
	...
	vendorService.SetCacheService(cacheService)
	customerService := services.NewCustomerService(customerRepo)
	customerService.SetCacheService(cacheService)
	activityPresetService := services.NewActivityPresetService(activityPresetRepo)
	activityPresetService.SetCacheService(cacheService)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/invoices", func(r chi.Router) {
				...
				r.With(middleware.RequirePermission("invoices.edit")).Post("/{id}/reschedule", invoiceHandler.RescheduleDueDate)
